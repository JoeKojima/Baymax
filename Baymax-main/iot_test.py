...
ENDPOINT   = os.environ["AWS_IOT_ENDPOINT"]          # e.g. xxxxx-ats.iot.us-east-1.amazonaws.com
...
SELF_TEST_PING   = True   # send a local ping after subscribe to verify RX/TX path
WILDCARD_SUB     = False  # set True to subscribe to "#" instead of TOPIC_CMDS for debugging
...
# ----------------------------------------------------------------------
# Outbound event helpers (device -> cloud)
# ----------------------------------------------------------------------
...
def on_message(topic, payload, dup, qos, retain, **kwargs):
    ...
    try:
        ...
        try:
            ...
            # If publishing fails, at least log locally
...
def connect_and_listen():
    ...
    # --- Region sanity check: we want us-east-1 ---
    if ".us-east-1." not in ENDPOINT:
        _log("WARNING: AWS_IOT_ENDPOINT does NOT look like us-east-1. Current endpoint:", ENDPOINT)
        _log("If you want us-east-1, set AWS_IOT_ENDPOINT to your us-east-1 IoT endpoint in .env")
    else:
        _log("Endpoint appears to be us-east-1:", ENDPOINT)
    ...
    try:
        sub_future, packet_id = mqtt_connection.subscribe(
            topic=sub_topic,
            qos=mqtt.QoS.AT_LEAST_ONCE,
            callback=on_message
        )
        sub_result = sub_future.result()  # raises on error
        _log("Subscribe OK:", sub_result)
    except Exception as e:
        _log("Subscribe FAILED:", repr(e))
        try:
            publish_error("subscribe_failed", {"error": str(e), "topic": sub_topic})
        except Exception:
            pass
        return
    ...
    # --- Optional self-test ping to verify RX/TX path on this device ---
    if SELF_TEST_PING:
        try:
            test_cmd = {"type": "ping", "req_id": "self-test"}
            mqtt_connection.publish(
                topic=TOPIC_CMDS,
                payload=json.dumps(test_cmd),
                qos=mqtt.QoS.AT_LEAST_ONCE,
            )
            _log("Published self-test ping to", TOPIC_CMDS)
        except Exception as e:
            _log("Failed to publish self-test ping:", repr(e))
